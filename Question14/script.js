    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "A") {
				feedback.textContent = "✔ Correct! Since a singly linked list has a reference to the head node, inserting a new node at the beginning only requires changing one pointer.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is A: Since a singly linked list has a reference to the head node, inserting a new node at the beginning only requires changing one pointer.";
				feedback.className = "wrong";
			}
        });
    } else {
