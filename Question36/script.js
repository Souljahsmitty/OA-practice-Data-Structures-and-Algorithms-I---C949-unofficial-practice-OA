    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! A hash table stores key-value pairs and allows O(1) average-time lookups.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is **B. A hash table stores key-value pairs and allows O(1) average-time lookups.";
				feedback.className = "wrong";
			}
        });
    } else {
