    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! Binary search divides the array into halves, so it only works efficiently when the array is sorted.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is B: Binary search divides the array into halves, so it only works efficiently when the array is sorted.";
				feedback.className = "wrong";
			}
        });
    } else {
