    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! Collisions in hash tables occur when multiple keys hash to the same index, requiring resolution techniques such as chaining or open addressing.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect.The correct answer is **B. Collisions in hash tables occur when multiple keys hash to the same index, requiring resolution techniques such as chaining or open addressing.";
				feedback.className = "wrong";
			}
        });
    } else {
